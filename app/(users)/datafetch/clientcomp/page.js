...
const DataFetchClient = () => {
  ...
  if (userInfo.error) {
    return (
      <div className="min-h-screen bg-linear-to-br from-red-50 via-orange-50 to-amber-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center border-2 border-red-200">
          <h1 className="text-2xl font-bold text-red-600 mb-2">API Error</h1>
          <p className="text-gray-700 font-medium">{userInfo.error}</p>
          <p className="text-sm text-gray-500 mt-4">Genderize API daily limit reached. Please try again later.</p>
        </div>
      </div>
    );
  }
